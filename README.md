...
复制前请按本机路径、shell 调整。**`~`** 在 Windows 上多为 `C:\Users\你的用户名`。

### Cursor CLI（上游）

| 目的 | 命令 |
| --- | --- |
| 查看版本 | `agent --version` 或 `cursor-agent --version` |
| 登录 | `agent login`（或 `cursor-agent login`） |
| 查安装位置（Windows） | `where.exe agent` 或 `where.exe cursor-agent` |
| 查安装位置（macOS/Linux） | `which agent` 或 `which cursor-agent` |

### 本适配器（npm / 源码）

| 目的 | 命令 |
| --- | --- |
| 全局安装 | `npm install -g fzx-cursor-acp` |
| 免全局、单次运行 | `npx -y fzx-cursor-acp@latest` |
| 克隆与本地构建 | `git clone https://github.com/ai-fzx/fzx_cursor_acp.git && cd fzx_cursor_acp && npm install && npm run build` |
| 直接跑构建产物 | `node dist/main.js` |
| 开发模式（热重载） | `npm run dev -- --cursor-command agent --permission-mode approve-reads` |
| 跑测试 | `npm test` |
| 调试日志（环境变量） | **PowerShell：** `$env:LOG_LEVEL="debug"; node dist/main.js` · **bash/zsh：** `LOG_LEVEL=debug node dist/main.js` |

### acpx（终端）

全局/本地安装 acpx 见 [openclaw/acpx](https://github.com/openclaw/acpx)（**`acpx` 与 `fzx-cursor-acp` 是不同包**：前者是 ACP 客户端，后者是本适配器）。配置文件：**`~/.acpx/config.json`**（Windows 常见：`%USERPROFILE%\.acpx\config.json`）。

| 目的 | 命令示例 |
| --- | --- |
| 新建会话（首次必做） | `acpx cursor sessions new` |
| 多轮对话 | `acpx cursor "你的问题"` |
| 指定工作目录 | `acpx --cwd D:\myrepo cursor "说明任务"` |
| 命名并行会话 | `acpx cursor sessions new --name api`，再 `acpx cursor -s api "只在该会话里继续"` |
| 会话列表 / 详情 / 历史 | `acpx cursor sessions list`、`sessions show`、`sessions history` |
| 软关闭会话 | `acpx cursor sessions close` |
| 状态（进程是否存活） | `acpx cursor status` |
| 一次性、不保留会话 | `acpx cursor exec "一句话说明要做什么"` |
| 排队且不等待当前轮结束 | `acpx cursor --no-wait "下一条"` |
| 取消当前轮 | `acpx cursor cancel` |
| 权限策略（全局选项） | `acpx --approve-all cursor "..."` · `acpx --approve-reads cursor "..."` · `acpx --deny-all cursor "..."` |

更全的子命令、标志见 acpx 文档：[docs/CLI.md](https://github.com/openclaw/acpx/blob/main/docs/CLI.md)（**alpha，接口可能变**）。

---

### acpx cursor：Plan 模式（`set-mode`）

Cursor 在 ACP 里支持的核心模式与 CLI 一致，包括 **`plan`**（偏规划、只读倾向）、**`agent`**（完整工具）、**`ask`**（问答/只读）。详见 [Cursor ACP 文档 · Modes](https://cursor.com/docs/cli/acp)。

在 **已有会话** 的前提下，可用 acpx 的 **`set-mode`** 切模式（走 ACP `session/set_mode`，**具体允许的 mode 字符串以 Cursor 适配器为准**；若报 `Invalid params` 说明当前版本不支持该值）：

```bash
acpx cursor sessions new
acpx cursor set-mode plan
acpx cursor "先帮我列一个实现计划，再决定要不要改代码"
```

切回默认「可改代码」风格时可尝试：

```bash
acpx cursor set-mode agent
```

若 `set-mode` 在你的 acpx / Cursor 版本组合下不可用，可在 **提示词里** 明确要求「只做 plan、不要直接改文件」，并配合 **`--approve-reads`** 降低写操作概率。

---

### 安装「acpx 技能包」（CLI 参考，不是 Cursor 自带技能）

acpx 仓库提供可安装的 **Skill**，方便 Pi / OpenClaw 等编排器查阅 **全部 acpx 子命令**：

```bash
npx acpx@latest --skill install acpx
```

安装后可在技能说明里打开完整命令表；在线只读：[skills/acpx/SKILL.md](https://raw.githubusercontent.com/openclaw/acpx/main/skills/acpx/SKILL.md)。

---

### Cursor 侧的 Agent Skills（项目目录里的技能）

这与 **`acpx --skill install`** 是两套东西：

| 类型 | 是什么 | 怎么用 |
| --- | --- | --- |
| **acpx skill** | acpx 自己的 **CLI 使用说明/工作流**，装到编排器能「会操作 acpx」 | `npx acpx@latest --skill install acpx` |
| **Cursor Agent Skills** | Cursor 定义的 **`.cursor/skills/`、`~/.cursor/skills/`** 等目录下的技能包（含 `SKILL.md`），教 **Cursor CLI / Agent** 做专项任务 | 按 [Agent Skills](https://www.cursor.com/docs/context/skills) 把技能放进项目或用户目录；在 **`acpx --cwd 你的仓库`** 下调用 `acpx cursor` 时，Cursor 会从该 **工作目录** 加载项目内上下文（与在 IDE 里打开该仓库类似） |

**MCP 服务器**（若要用）：Cursor ACP 说明里写支持项目或用户级 **`.cursor/mcp.json`**，需在对应目录启动、并按 Cursor 文档完成授权。详见 [Cursor ACP · MCP servers](https://cursor.com/docs/cli/acp)。
...
| --- | --- |
| 健康检查 | `/acp doctor` |
| 新建 Cursor 会话 | **`/acp spawn cursor`**（必须带 `cursor`） |
